...
def main
  options = ARGV.getopts('l', 'r', 'a')
  path    = ARGV[0] || Dir.pwd
  ls(path, option_a: options['a'], option_r: options['r'], option_l: options['l'])
end

def ls(path, option_a: false, option_r: false, option_l: false)
  files = read_files(path, option_a: option_a, option_r: option_r)
...
def read_files(path, option_a: false, option_r: false)
  ...
  if File.directory?(path)
    ...
    files        = Dir.glob('*', dotfile_flag, base: path)
